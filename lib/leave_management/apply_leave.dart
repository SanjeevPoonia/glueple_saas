...
class _ApplyLeaveScreen extends State<ApplyLeaveScreen> {
  ...
  _submitLeaveOnServer(List<dynamic> leaveDetails) async {
    ...
    for (var leaveDetail in leaveDetails) {
      requestModel.addAll(leaveDetail);
    }
    ...
    try {
      var response = await dio.post(apiUrl, data: requestModel);
      Navigator.pop(context);
      print("Submit Leave Response: ${response.data}");

      // Updated condition to match the API response format
